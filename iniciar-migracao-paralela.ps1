...
if (-not $SkipDump) {
    ...
    if (-not $pgDump) {
        ...
        $dumpCmd = "docker run --rm --network host -e PGPASSWORD=password -v ${PWD}:/backup postgres:17 pg_dump -h 192.168.15.22 -U postgres -Fc cnpj_processado -f /backup/cnpj_processado.dump"
    } else {
